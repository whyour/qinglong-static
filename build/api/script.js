...
exports.default = (app) => {
    ...
    route.post('/', upload.single('file'), async (req, res, next) => {
        ...
        try {
            ...
            if (directory) {
                fs.mkdirSync((0, path_1.join)(path, directory), { recursive: true });
                return res.send({ code: 200 });
            }
            if (!originFilename) {
            ...
            if (fs.existsSync(originFilePath)) {
                fs.copyFileSync(originFilePath, (0, path_1.join)(config_1.default.bakPath, originFilename.replace(/\//g, '')));
