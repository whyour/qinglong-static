...
exports.default = (app) => {
    ...
    route.get('/:dir/:file', async (req, res, next) => {
        const logger = typedi_1.Container.get('logger');
        try {
            const { dir, file } = req.params;
            const content = (0, util_1.getFileContentByName)(`${config_1.default.logPath}/${dir}/${file}`);
    ...
    route.get('/:file', async (req, res, next) => {
        ...
        try {
            ...
            const content = (0, util_1.getFileContentByName)(`${config_1.default.logPath}/${file}`);
            res.send({ code: 200, data: content });
        }
        catch (e) {
            logger.error('🔥 error: %o', e);
