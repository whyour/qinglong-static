...
let SubscriptionService = class SubscriptionService {
    ...
    handleTask(doc, needCreate = true, needAddKey = true) {
        ...
        if (doc.schedule_type === 'crontab') {
            ...
            needCreate &&
                this.scheduleService.createCronTask(doc, this.taskCallbacks(doc));
        }
        else {
            this.scheduleService.cancelIntervalTask(doc);
            const { type, value } = doc.interval_schedule;
            needCreate &&
                this.scheduleService.createIntervalTask(doc, { [type]: value }, true, this.taskCallbacks(doc));
        }
    }
    ...
    taskCallbacks(doc) {
        return {
            onStart: async (cp, startTime) => {
                ...
                try {
                    if (doc.sub_before) {
                        beforeStr = (0, child_process_1.execSync)(doc.sub_before).toString();
                    }
                }
                catch (error) {
                ...
                await this.handleLogPath(logPath, `${beforeStr}## 开始执行... ${startTime.format('YYYY-MM-DD HH:mm:ss')}\n`);
            ...
            onEnd: async (cp, endTime, diff) => {
                ...
                try {
                    if (sub.sub_after) {
                        afterStr = (0, child_process_1.execSync)(sub.sub_after).toString();
                    }
                }
                catch (error) {
