...
let SubscriptionService = class SubscriptionService {
    ...
    formatCommand(doc, url) {
        ...
        const { type, whitelist, blacklist, dependences, branch, extensions, proxy, } = doc;
        if (type === 'file') {
            command += `raw "${_url}"`;
        }
        else {
            command += `repo "${_url}" "${whitelist || ''}" "${blacklist || ''}" "${dependences || ''}" "${branch || ''}" "${extensions || ''}" "${proxy || ''}"`;
        }
        return command;
    }
    formatUrl(doc) {
    ...
    async handleTask(doc, needCreate = true, needAddKey = true, runImmediately = false) {
        ...
        if (doc.type === 'private-repo' && doc.pull_type === 'ssh-key') {
            if (needAddKey) {
                this.sshKeyService.addSSHKey(doc.pull_option.private_key, doc.alias, host, doc.proxy);
            }
            else {
