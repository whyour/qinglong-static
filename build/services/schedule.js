...
let ScheduleService = class ScheduleService {
    ...
    async createCronTask({ id = 0, command, name, schedule = '' }) {
        ...
        this.scheduleStacks.set(_id, node_schedule_1.default.scheduleJob(id + '', schedule, async () => {
            try {
                (0, child_process_1.exec)(command, { maxBuffer: this.maxBuffer }, async (error, stdout, stderr) => {
                    if (error) {
                        await this.logger.error('执行任务%s失败，时间：%s, 错误信息：%j', command, new Date().toLocaleString(), error);
                    }
                    if (stderr) {
                        await this.logger.error('执行任务%s失败，时间：%s, 错误信息：%j', command, new Date().toLocaleString(), stderr);
                    }
                });
            }
            catch (error) {
    ...
    async createIntervalTask({ id = 0, command, name = '' }, schedule) {
        ...
        const job = new toad_scheduler_1.LongIntervalJob(Object.assign({}, schedule), task, _id);
