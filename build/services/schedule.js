...
let ScheduleService = class ScheduleService {
    ...
    async generateSchedule({ id = 0, command, name, schedule }) {
        this.logger.info('[创建定时任务]，任务ID: %s，cron: %s，任务名: %s，执行命令: %s', id, schedule, name, command);
        this.scheduleStacks.set(id, node_schedule_1.default.scheduleJob(id + '', schedule, async () => {
            try {
    ...
    async cancelSchedule({ id = 0, name }) {
        var _a;
        this.logger.info('[取消定时任务]，任务名：%s', name);
        this.scheduleStacks.has(id) && ((_a = this.scheduleStacks.get(id)) === null || _a === void 0 ? void 0 : _a.cancel());
    }
};
