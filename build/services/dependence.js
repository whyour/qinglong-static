...
let DependenceService = class DependenceService {
    ...
    async cancel(ids) {
        ...
        for (const doc of docs) {
            pLimit_1.default.removeQueuedDependency(doc);
            const depInstallCommand = (0, util_1.getInstallCommand)(doc.type, doc.name);
            const depUnInstallCommand = (0, util_1.getUninstallCommand)(doc.type, doc.name);
    ...
    installOrUninstallDependency(dependency, isInstall = true, force = false) {
        return pLimit_1.default.runDependeny(dependency, () => {
            return new Promise(async (resolve) => {
                ...
                // 判断是否已经安装过依赖
                if (isInstall && !force) {
                    const getCommand = (0, util_1.getGetCommand)(dependency.type, depName);
