...
let DependenceService = class DependenceService {
    ...
    async dependencies({ searchValue, type, status, }, sort = [], query = {}) {
        let condition = query;
        const dependenceType = type && dependence_1.DependenceTypes[type] !== undefined
            ? dependence_1.DependenceTypes[type]
            : undefined;
    ...
    async refreshInstalledStatuses(type) {
        const cacheDependenceTypes = [
            dependence_1.DependenceTypes.nodejs,
        ...
        const docs = await dependence_1.DependenceModel.findAll({
            where: {
                ...
                type: type === undefined ? { [sequelize_1.Op.in]: cacheDependenceTypes } : type,
            },
        });
        const checks = await (0, util_1.concurrentRun)(docs.map((doc) => async () => {
            return (await this.isDependenceInstalled(doc)) ? undefined : doc.id;
        }), 5);
        const missingIds = (checks || []).filter((id) => id !== undefined);
        if (missingIds.length) {
            await dependence_1.DependenceModel.update({ status: dependence_1.DependenceStatus.installFailed }, { where: { id: missingIds } });
        }
    }
    async isDependenceInstalled(dependency) {
        var _a;
        ...
        const depInfo = (await (0, util_1.promiseExecSuccess)((0, util_1.getGetCommand)(dependency.type, depName)))
            .replace(/\s{2,}/, ' ')
            .replace(/\s+$/, '');
        const nameMatches = (dependency.type === dependence_1.DependenceTypes.nodejs &&
            ((_a = depInfo.split(' ')) === null || _a === void 0 ? void 0 : _a[0]) === depName) ||
            dependency.type === dependence_1.DependenceTypes.python3;
        return Boolean(depInfo && nameMatches && (!depVersion || depInfo.includes(depVersion)));
    }
    ...
    async cancel(ids) {
        ...
        for (const doc of docs) {
            ...
            const pids = await Promise.all([
                (0, util_1.getPid)(depInstallCommand),
                (0, util_1.getPid)(depUnInstallCommand),
            ]);
