...
let SystemService = class SystemService {
    ...
    async getSystemConfig() {
        ...
        return Object.assign(Object.assign({}, doc), { info: Object.assign(Object.assign({}, doc.info), { timezone: ((_a = doc.info) === null || _a === void 0 ? void 0 : _a.timezone) || 'Asia/Shanghai' }) });
    }
    ...
    async notify({ title, content, notificationInfo, }) {
        const typeString = typeof (notificationInfo === null || notificationInfo === void 0 ? void 0 : notificationInfo.type) === 'number'
            ? const_1.NotificationModeStringMap[notificationInfo.type]
            : undefined;
    ...
    async exportData(res, type) {
        try {
            ...
            await (0, util_1.promiseExec)(`cd ${config_1.default.dataPath} && cd ../ && tar -zcvf ${config_1.default.dataTgzFile} ${dataPaths.join(' ')}`);
    ...
    async getSystemLog(res, query) {
        ...
        const limit = Math.min(Math.max(Number(query.limit) || 1024 * 1024, 1), 1024 * 1024);
        ...
        for (let index = logs.length - 1; index >= 0 && remaining > 0; index--) {
            ...
            if (length > 0) {
                selected.unshift(Object.assign(Object.assign({}, log), { start: size - length, length }));
                remaining -= length;
            }
        }
    ...
    async updateTimezone(info) {
        if (!info.timezone) {
            info.timezone = 'Asia/Shanghai';
        }
        const oDoc = await this.getSystemConfig();
        await this.updateAuthDb(Object.assign(Object.assign({}, oDoc), { info: Object.assign(Object.assign({}, oDoc.info), info) }));
        const success = await (0, util_1.setSystemTimezone)(info.timezone);
        if (success) {
            return { code: 200, data: info };
        }
        else {
    ...
    async cleanDependence(type) {
        ...
        await fs_1.default.promises.rm(finalPath, { recursive: true, force: true });
        await dependence_1.DependenceModel.update({ status: dependence_1.DependenceStatus.installFailed }, {
            where: {
                type: type === 'node' ? dependence_1.DependenceTypes.nodejs : dependence_1.DependenceTypes.python3,
                status: dependence_1.DependenceStatus.installed,
            },
        });
        return { code: 200 };
