...
let NotificationService = class NotificationService {
    ...
    async goCqHttpBot() {
        ...
        const res = await got_1.default
            .post(`${goCqHttpBotUrl}?access_token=${goCqHttpBotToken}&${goCqHttpBotQq}`, {
    ...
    async bark() {
        ...
        const url = `${barkPush}/${encodeURIComponent(this.title)}/${encodeURIComponent(this.content)}?sound=${barkSound}&group=${barkGroup}`;
        const res = await got_1.default
            .get(url, {
            timeout: this.timeout,
            retry: 0,
    ...
    async weWorkApp() {
        ...
        const { access_token } = await got_1.default
            .post(url, {
            timeout: this.timeout,
            retry: 0,
            json: {
...
__decorate([
    typedi_1.Inject((type) => user_1.default),
