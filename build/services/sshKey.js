...
let SshKeyService = class SshKeyService {
    ...
    generatePrivateKeyFile(alias, key) {
        try {
            fs_1.default.writeFileSync(`${this.sshPath}/${alias}`, key, { encoding: 'utf8' });
        }
        catch (error) {
    ...
    generateSshConfig(configs) {
        try {
            for (const config of configs) {
                fs_1.default.appendFileSync(this.sshConfigFilePath, config, { encoding: 'utf8' });
            }
        }
        catch (error) {
    ...
    removeSshConfig(config) {
        try {
            fs_1.default.readFileSync(this.sshConfigFilePath, { encoding: 'utf8' }).replace(config, '');
        }
        catch (error) {
